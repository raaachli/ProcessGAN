...
if __name__ == "__main__":
    ...
    file = 'gru_pgan.csv'
    seq_path = 'log/INT/'
    alignment_path = 'log/INT/Alignment/'
    cs_path = 'log/INT/Consensus_Sequence/'
    threshold = 0.3
    ...
    # mean_len = 13
    # cs_path = "log/" + data + "/Consensus_Sequence/"
    # os.makedirs(os.path.dirname(cs_path), exist_ok=True)
    # for i in range(5, 100, 5):
    #     threshold = i/100
    #     aut_seq_path = 'log/'+data+'/'+data+'.csv'
    #     aut_ali_path = 'log/'+data+'/Alignment/'+data+'.csv'
    #     aut_cs_path = 'log/' + data + '/Consensus_Sequence/' + data + '.csv'
    #     aut_cs = get_cs_from_alignment(aut_seq_path, aut_ali_path, threshold)
    #     if len(aut_cs)<mean_len+0:
    #         print(threshold)
    #         print(len(aut_cs))
    #         with open(aut_cs_path, 'w') as csvfile:
    #             cw = csv.writer(csvfile, delimiter=',')
    #             for act in aut_cs:
    #                 cw.writerow([act])
    #         break
    ...
    # seq_path = 'log/'+data+'/'
    # save_ed_path = "log/" + data + "/ed_result.csv"
    # other_file_names = [data + '.csv', 'Alignment', 'Consensus_Sequence', 'ed_result.csv']

    # mean = [27, 13, 26, 33, 36, 21, 34, 35, 30] # AIR
    # mean = [41, 32, 39, 44, 45, 40, 48, 45, 50] # SS
    # mean = [5, 4, 5, 5, 6, 5, 6, 6, 6] # ED
    # mean = [11, 11, 12, 11, 13, 11, 13, 13, 12] # INT
    #
    # j = 0
    # for file in os.listdir(seq_path):
    #     if file not in other_file_names:
    #         print(file)
    #         gen_seq_path = seq_path + file
    #         gen_ali_path = seq_path +'/Alignment/'+ file
    #         save_gen_cs_path = cs_path + file
    #         for i in range(5, 100, 5):
    #             threshold_2 = i / 100
    #             gen_cs = get_cs_from_alignment(gen_seq_path, gen_ali_path, threshold_2)
    #             if len(gen_cs) < mean[j] + 0:
    #                 print(threshold_2)
    #                 print(len(gen_cs))
    #                 with open(save_gen_cs_path, 'w') as csvfile:
    #                     cw = csv.writer(csvfile, delimiter=',')
    #                     for act in gen_cs:
    #                         cw.writerow([act])
    #                 break
    #         ins, dele = get_ins_del(aut_cs, gen_cs)
    #         ed = get_ed(aut_cs, gen_cs)
    #         print(ed)
    #         with open(save_ed_path, 'a') as csvfile:
    #             cw = csv.writer(csvfile, delimiter=',')
    #             cw.writerow([file, ins, dele, ed])
    #         j+=1
    # datas=['ED']
    # threshold = 0.2
    # for data in datas:
    ...
    #     seq_path = 'log/' + data + '/'
    #
    #     alignment_path = "log/" + data + "/Alignment/"
    #     os.makedirs(os.path.dirname(alignment_path), exist_ok=True)
    #
    #     cs_path = "log/" + data + "/Consensus_Sequence/"
    #     os.makedirs(os.path.dirname(cs_path), exist_ok=True)
    #
    #     save_aut_ali_path = alignment_path + data + ".csv"
    #     save_aut_cs_path = cs_path + data + ".csv"
    #     aut_cs = train(aut_seq_path, save_aut_ali_path, save_aut_cs_path, threshold)
    #
    #     save_ed_path = "log/" + data + "/ed_result.csv"
    #     other_file_names = [data+'.csv', 'Alignment', 'Consensus_Sequence', 'ed_result.csv']
    #     for file in os.listdir(seq_path):
    ...
    #
    #             end_time = time.time()
    #             print(datetime.timedelta(seconds=(end_time - s_time)))

    # train(filename = "/Users/rache/Desktop/nih_project/IV_PIMA/IV.csv")
