...
class ProcessGAN:
    def __init__(self, res_path, save_path, config, gen_num, mode):
        ...
        Parameters:
            'w_a'       : the weight of activity distribution loss
    ...
    def get_pre_exp_loss(self, pre_epoch, g_model, d_model, train_dataloader):
        """When activity distribution loss is added, calculate the expectation loss values first.
        The loss values are expected in the same scale when added together.
        """
        mean_act_loss = 0
        mean_gen_loss = 0
        d_criterion = nn.BCELoss()
