...
class Transformer_NAR:
            ...
            for j in range(self.seq_len + 1):
                for k in range(ntokens + 1):
                    if out_list[0][j][k] == 1:
                        seq.append(k)
            sub_samp = []
            # sub_samp_all = [] # see all tokens without remove end token
            n = len(seq)
            for j in range(n):
                tok = seq[j]
                # sub_samp_all.append(str(tok))
                if tok != ntokens:
    ...
    def train(self, target):
        ...
        g_model = TransformerModel(self.n_inp, self.emb_size, self.n_head, self.n_hid,
                                   self.n_layer, self.pad_ind, self.drop_out).to(self.device)
        criterion = nn.CrossEntropyLoss()
        optimizer = torch.optim.SGD(g_model.parameters(), lr=self.lr_gen)

        dataset = load_nar_data(target)
        train_data, val_data, test_data = torch.utils.data.random_split(dataset, (self.train_size, self.valid_size, self.test_size),
